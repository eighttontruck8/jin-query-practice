...
-- 아래에 쿼리를 작성하세요.
SELECT STM.IPSIYEAR "입학연도", 모집시기, STM.SELTYPENAME "전형명"
, STM.MajorName "모집학과", STM.MAJORPERSONNEL "모집인원"
, 지원인원
FROM VWSelectTypeModel STM

LEFT JOIN (
	SELECT IPSIYEAR "입학연도", IPSIGUBUN "모집시기", GUNID "모집군", SELTYPECODE "전형코드", MAJORCODE "학과코드"
	, ISNULL(COUNT(*), 0) "지원인원"
	FROM APPLYINFO
	GROUP BY IPSIYEAR, IPSIGUBUN, GUNID, SELTYPECODE, MAJORCODE
) AS AI
ON STM.IPSIYEAR = 입학연도 AND STM.IPSIGUBUN = 모집시기 AND STM.GunID = 모집군 AND STM.SelTypeCode = 전형코드 AND STM.MajorCode = 학과코드
WHERE IPSIYEAR = 2024 AND 모집시기 = 1
ORDER BY 전형코드, 학과코드;
