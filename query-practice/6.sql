...
-- 아래에 쿼리를 작성하세요.
SELECT STM.IPSIYEAR "입학연도", STM.IPSINAME "모집시기", STM.SELTYPENAME "전형명", STM.MAJORNAME "모집학과", ISNULL(STM.MAJORPERSONNEL,0) "모집인원"
, ISNULL(AI.지원인원, 0) "지원인원"
, CASE WHEN STM.MAJORPERSONNEL IS NULL OR STM.MAJORPERSONNEL = 0 OR AI.지원인원 = 0 OR AI.지원인원 IS NULL THEN 0 ELSE ROUND(AI.지원인원 * 1.0 / STM.MAJORPERSONNEL, 2) END "경쟁률" 
